...
class Telegram_Bot:
    ...
    def __init__(self, conf_path):
        ...
        MessageLoop(
            self.bot,
            {"chat": self.on_chat_message, "callback_query": self.on_callback_query},
        ).run_as_thread()
        self.cat_info = {"ip": self.conf["CatIP"], "port": self.conf["CatPort"]}
        ...
        self.greenhouse = {
            ...
            "devices": [],
            "usrID": "",
            "gh_params": {"temperature": "", "CO2_level": "", "humidity": ""},
            "plantsList": [],
            "lastUpdate": "",
        }
            ...
            if len(parameters) == 1:
                if command == "/signup":
                    if self.userConnected == False:
                        ...
                        try:
                            req = requests.post(
                                self.addr_cat + "/addUser", json.dumps(newUser)
                            )
                            req_id = requests.get(self.addr_cat + "/user/recentID")
                            self.bot.sendMessage(
                                chat_ID, text=f"User {parameters[0]} correctly created."
                            )
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"Your ID is {req_id.json()['usrID']}"
                            ...
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"Sorry, in this moment the catalog web service is unreachable! Try again later.",
                            )

                    else:
                        self.bot.sendMessage(
                            chat_ID,
                            text=f"You are already logged with user {self.user['name']} {self.user['usrID']}.",
                        )
                    ...
                    if self.userConnected == True:
                        if self.grHselected == True:
                            try:
                                req_dev = requests.get(
                                    self.addr_cat + f"/device?devID={parameters[0]}"
                                )
                                r_assigned_dev = requests.get(
                                    self.addr_cat + f"/greenhouse?devID={parameters[0]}"
                                )
                                if req_dev.ok:
                                    device = req_dev.json()
                                    if not r_assigned_dev.ok:
                                        ...
                                        req_gh = requests.put(
                                            self.addr_cat + "/updateGreenhouse",
                                            json.dumps(self.greenhouse),
                                        )
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"Device {parameters[0]} correctly added"
                                        ...
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"The device {parameters[0]} is already used by"
                                            " another greenhouse!",
                                        )
                                    ...
                                    self.bot.sendMessage(
                                        chat_ID,
                                        text=f"The device {parameters[0]} does not exist!",
                                    )
                            except:
                                self.bot.sendMessage(
                                    chat_ID,
                            ...
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"Greenhouse not selected."
                        ...
                        self.bot.sendMessage(
                            chat_ID,
                            text=f"User not connected."
                            "\nTo add a greenhouse plant, first you need to sign in with a user clicking on /start",
                        )
                else:
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"Number of parameters for command {command} not correct!",
                    )
                ...
                if command == "/signin":
                    if self.userConnected == False:
                        ...
                        try:
                            req = requests.get(
                                self.addr_cat + f"/user?usrID={self.user['usrID']}"
                            )
                            ...
                            if req.ok:
                                ...
                                if usr["name"] == self.user["name"]:
                                    self.bot.sendMessage(
                                        chat_ID,
                                        text=f"Logged in with user: {parameters[0]} {parameters[1]}",
                                    )
                                    self.userConnected = True
                                    if usr["ghID"] == []:
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"No greenhouses found, add your first greenhouse."
                                            "\n /addgreenhouse",
                                        )
                                    else:
                                        ...
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"Select one greenhouse /selectgreenhouse",
                                        )
                                else:
                                    self.bot.sendMessage(
                                        chat_ID,
                                        text=f"The name {self.user['name']} does not match\
                                                                                            with the existing user!",
                                    )
                            else:
                                self.bot.sendMessage(
                                    chat_ID,
                                    text=f"User {self.user['usrID']} not found!",
                                )
                        except:
                            self.bot.sendMessage(
                                chat_ID,
                        ...
                        buttons = [
                            [
                                InlineKeyboardButton(
                                    text=f"Yes", callback_data=f"signout"
                                ),
                        ...
                        self.bot.sendMessage(
                            chat_ID,
                            text=f"Do you want to log out and enter with another user?",
                            parse_mode="Markdown",
                ...
                # Add a owned plant to the selected greenhouse
                    ...
                    if self.userConnected == True:
                        if self.grHselected == True:
                            ...
                            used_sens = [
                                plant["sensID"]
                                for plant in self.greenhouse["plantsList"]
                            ...
                            out = False
                            try:
                                req_plant = requests.get(
                                    self.addr_cat
                                    + f"/user?usrID={self.user['usrID']}&plant={plant_name}"
                                )
                                req_sens = requests.get(
                                    self.addr_cat + f"/device/sensors?devID={device}"
                                )
                                req_act = requests.get(
                                    self.addr_cat + f"/device/actuators?devID={device}"
                                )
                                ...
                                if req_sens.ok and req_act.ok:
                                    ...
                                    free_sens = [
                                        ...
                                        if sens not in used_sens
                                    ]
                                    free_act = [
                                        act for act in all_act if act not in used_act
                                    ]
                                    ...
                                    try:
                                        ...
                                        actID = free_act.pop(
                                            free_act.index("a" + sensID[1:])
                                        )
                                    except:
                                        out = True
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"Device {device} does not have more free sensor!",
                                        )
                                else:
                                    self.bot.sendMessage(
                                        chat_ID, text=f"Device {device} does not exist!"
                                    )
                                    ...
                                    if req_plant.ok:
                                        ...
                                        if sensID and actID:
                                            ...
                                            try:
                                                ...
                                                self.bot.sendMessage(
                                                    chat_ID,
                                                    text=f"{plant_name} correctly added "
                                            ...
                                            self.bot.sendMessage(
                                                chat_ID,
                                                text=f"There are no more free sensor in device {device}!",
                                            )
                                    else:
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"{plant_name} does not exist in user {self.user['usrID']} owned plants!",
                                        )
                ...
                if command == "/addplant":
                    if self.userConnected == True:
                        if parameters[1].isnumeric() and parameters[2].isnumeric():
                            ...
                            if (
                                parameters[1] >= 0
                                and parameters[1] <= 100
                                and parameters[2] >= 0
                                and parameters[2] <= 100
                            ):
                                if parameters[1] < parameters[2]:
                                    ...
                                    try:
                                        req = requests.post(
                                            self.addr_cat
                                            + f"/addPlant?usrID={self.user['usrID']}",
                                            data=json.dumps(newPlant),
                                        )
                                        if req.ok:
                                            self.bot.sendMessage(
                                                chat_ID,
                                                text=f"{newPlant['plant']} correctly added to the user database.",
                                            )
                                        else:
                                            self.bot.sendMessage(
                                                chat_ID,
                                                text=f"{newPlant['plant']} has not been added to the user database!.",
                                            )
                                    except:
                                        self.bot.sendMessage(
                                            chat_ID,
                                    ...
                                    self.bot.sendMessage(
                                        chat_ID,
                                        text="Low humidity threshold cannot be higher than the high treshold."
                                        "\nPlease reinsert the command.",
                                    )
                            else:
                                self.bot.sendMessage(
                                    chat_ID,
                                    text="Humidity threshold number out of range."
                                    "\nPlease reinsert the command.",
                                )
                        else:
                            self.bot.sendMessage(
                                chat_ID,
                                text="Humidity threshold format not correct."
                                "\nPlease reinsert the command.",
                            )
                    else:
                        self.bot.sendMessage(
                    ...
                    if self.userConnected:
                        ...
                        if parameters[1].isnumeric() and parameters[2].isnumeric():
                            ...
                            if (
                                ...
                                try:
                                    req_dev = requests.get(
                                        self.addr_cat + f"/device?devID={devID_gh}"
                                    )
                                    r_assigned_dev = requests.get(
                                        self.addr_cat + f"/greenhouse?devID={devID_gh}"
                                    )
                                    if req_dev.ok:
                                        device = req_dev.json()
                                        ...
                                        if not r_assigned_dev.ok:
                                            self.greenhouse["usrID"] = self.user[
                                                "usrID"
                                            ]
                                            ...
                                            req_gh = requests.post(
                                                self.addr_cat + "/addGreenhouse",
                                                json.dumps(self.greenhouse),
                                            )
                                            req_id = requests.get(
                                                self.addr_cat + "/greenhouse/recentID"
                                            )
                                            self.greenhouse["ghID"] = req_id.json()[
                                                "ghID"
                                            ]
                                            # Update user and device adding the new greenhouse
                                            self.user["ghID"].append(
                                                self.greenhouse["ghID"]
                                            )
                                            req_usr = requests.put(
                                                self.addr_cat + f"/updateUser",
                                                json.dumps(self.user),
                                            )
                                            req_dev = requests.put(
                                                self.addr_cat + f"/updateDevice",
                                                json.dumps(device),
                                            )
                                            ...
                                            if req_gh.ok and req_usr.ok:
                                                self.bot.sendMessage(
                                                    chat_ID,
                                                    text=f"Greenhouse {self.greenhouse['ghID']} correctly added."
                                                ...
                                                self.bot.sendMessage(
                                                    chat_ID,
                                                    text=f"Greenhouse {self.greenhouse['ghID']} has not been added!",
                                                )

                                        else:
                                            self.bot.sendMessage(
                                                chat_ID,
                                                text=f"The device {devID_gh} is already used by"
                                                " another greenhouse!",
                                            )
                                        ...
                                        self.bot.sendMessage(
                                            chat_ID,
                                            text=f"The device {devID_gh} does not exist!",
                                        )
                                except:
                                    self.bot.sendMessage(
                                        chat_ID,
                                ...
                                self.bot.sendMessage(
                                    chat_ID,
                                    text="Humidity and temperature values out of maximum range."
                                    "\nPlease reinsert the command.",
                                )
                        else:
                            self.bot.sendMessage(
                                chat_ID,
                                text="Humidity and temperature threshold format not correct."
                                "\nPlease reinsert the command.",
                            )
        ...
        ###################################
            ...
            if self.userConnected == True:
                if self.grHselected == True:
                    ...
                    if message == "/status":
                        ...
                        try:
                            req_ghINFO = requests.get(
                                self.addr_DA
                                + f"/getAllLastValues?ghID={self.greenhouse['ghID']}"
                            )
                            ghINFO = req_ghINFO.json()
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"In greenhouse {self.greenhouse['ghID']} there are:"
                            ...
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"Sorry, in this moment the data analysis web service is unreachable! Try again later.",
                            )
                    ...
                    # Check and print all the plant list of the selected greenhouse
                        ...
                        if len(self.greenhouse["plantsList"]) > 0:
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"The list of plants of greenhouse {self.greenhouse['ghID']} is:",
                            )
                            ...
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"There are no plants in greenhouse {self.greenhouse['ghID']}"
                    ...
                    # Inform the user how to obtain all the name of the plants of the selected greenhouse
                        ...
                        self.bot.sendMessage(
                            chat_ID,
                            text="If you want to select a specific plant obtain the greenhouse list with /plants",
                        )
                    ...
                    # If we have a greenhouse selected and we receive a command message of a plant present in this greenhouse, then this plant is selected
                        ...
                        self.greenhouse, "plantsList", "plant", message[1:]
                    ):
                        done = True
                        self.bot.sendMessage(
                            chat_ID, text=f"{message[1:]} plant selected"
                        )
                        self.plantSelected = True
                        self.plant["plant"] = message[1:]
                        ind, plant = searchDict(
                            self.greenhouse,
                            "plantsList",
                            "plant",
                            self.plant["plant"],
                    ...
                    # Inform the user to the complete command to add a plant to the greenhouse
                        ...
                        self.bot.sendMessage(
                            chat_ID,
                            text="To add a plant to the greenhouse write:"
                        ...
                        for dev in self.greenhouse["devices"]:
                            ...
                            try:
                                req_sens = requests.get(
                                    self.addr_cat
                                    + f"/device/sensors?devID={dev['devID']}"
                                )
                                ...
                                if req_sens.ok:
                                    ...
                                    self.bot.sendMessage(
                                        chat_ID,
                                        text=f"Device connector {dev['devID']} have {len(free_sens)} sensors available",
                                    )
                                else:
                                    self.bot.sendMessage(
                                        chat_ID,
                                        text=f"Device {dev['devID']} does not exist in the catalog!",
                                    )
                            except:
                                self.bot.sendMessage(
                                    chat_ID,
                                    text=f"Sorry, the catalog web service is unreachable! Try again later.",
                                )
                        ...
                        buttons = [
                            [
                                InlineKeyboardButton(
                                    text=f"Yes", callback_data=f"plants"
                                ),
                                InlineKeyboardButton(text=f"No", callback_data=f"none"),
                            ]
                        ]
                    ...
                    # Inform the user to the complete command to add a new device to the greenhouse
                        ...
                        self.bot.sendMessage(
                            chat_ID,
                            text="To add a new device to the selected greenhouse write:"
                    ...
                    if self.plantSelected == True:
                        ...
                        if message == "/plantstatus":
                            ...
                            try:
                                req_plantINFO = requests.get(
                                    self.addr_DA
                                    + f"/getLastMoistureLevel?ghID={self.greenhouse['ghID']}&sensID={self.plant['sensID']}"
                                )
                                plantINFO = req_plantINFO.json()
                                self.bot.sendMessage(
                                    chat_ID,
                                    text=f"Plant {self.plant['plant']} information are:"
                ...
                # Message for adding a new greenhouse
                    ...
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"To add a new greenhouse type the command:"
                ...
                # If present print of all the greenhouse of the user selected
                    ...
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"{self.user['name']} you have {len(self.user['ghID'])} greenhouses."
                ...
                # If the message is one of the greenhouse in the user list then is selected
                    ...
                    self.bot.sendMessage(
                        chat_ID, text=f"Greenhouse {message[1:]} selected"
                    )
                    ...
                    try:
                        req = requests.get(
                            self.addr_cat
                            + f"/greenhouse?ghID={self.greenhouse['ghID']}"
                        )
                        if req.ok:
                            ...
                            self.bot.sendMessage(
                                chat_ID,
                                text=f"An error occured when selecting greenhouse {self.greenhouse['ghID']}!",
                            )
                    except:
                        self.bot.sendMessage(
                            chat_ID,
                ...
                # Used to specify the correct command to add a new plant to the user database
                    ...
                    self.bot.sendMessage(
                        chat_ID,
                        text="Add a new plant to the user database."
                ...
                # Used to signout the user
                    ...
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"User correctly signed out."
                    ...
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"To acces nodered click on the following link:\n"
                        f"{link['link']}",
                    )
                ...
                # If greenhouse or plant commands are received before a greenhouse is selected send the proper error message
                    ...
                    message == "/addgrhouseplant"
                    or message == "/status"
                    or message == "/plants"
                ) and not self.grHselected:
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"No greenhouse selected."
                        "\nPlease first select a greenhouse with /selectgreenhouse"
                        "\nOr create a new one with /addgreenhouse",
                    )
                    ...
                    message == "/selectplant"
                    or message == "/plantstatus"
                    or message == "/addgrhousedevice"
                ) and not self.grHselected:
                    self.bot.sendMessage(
                        chat_ID,
                        text=f"No greenhouse selected."
                    ...
                    self.bot.sendMessage(
                        chat_ID,
                        text="Word/words not recognized. Please start with command /start",
                    )
            ...
            ##############################################
                ...
                ##########################
                    ...
                    message == "/addgreenhouse"
                    or message == "/selectgreenhouse"
                    or message == "/addplant"
                ):
                    self.bot.sendMessage(
                        chat_ID,
    ...
    def on_callback_query(self, msg):
        ...
        if action == "signup":
            self.bot.sendMessage(
                chat_ID, text="To sign up write it in this way:\n" "/signup_Name \n"
            )
        ...
        # When the signin button is pressed
            ...
            self.bot.sendMessage(
                chat_ID,
                text="To sign in into your account write it in this way:\n"
                "/signin_Name_ID \n",
            )
            ...
            try:
                ...
                if req_users.ok:
                    ...
                    if len(ownedPlants) != 0:
                        ...
                        self.bot.sendMessage(
                            chat_ID,
                            text=f"User {self.user['usrID']} does not have plants in its database!"
                            "\nTo add a new plant to the user databese click /addplant",
                        )
            except:
                self.bot.sendMessage(
                    chat_ID,
    ...
    def help(self, chat_ID):
        ...
        if self.userConnected == True and self.grHselected == False:
            ...
            self.userConnected == True
            and self.grHselected == True
            and self.plantSelected == False
        ):
            help_message = (
                "*You can perform the following actions:*\n"
                "- /signout: To log out from this user\n"
            ...
            help_message = (
                "*Before doing anything please log in into your account*\n"
                "- /start: Sign in or sign up commands\n"
            )

        self.bot.sendMessage(chat_ID, text=help_message, parse_mode="Markdown")
    ...
    def start(self, chat_ID):
        ...
        if self.userConnected == True:
            message = (
                "Welcome to the SmartGreenHouse bot 🌱"
            ...
            buttons = [
                [
                    InlineKeyboardButton(text=f"Sign up", callback_data=f"signup"),
                    InlineKeyboardButton(text=f"Sign in", callback_data=f"signin"),
                ]
            ]
            keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
            self.bot.sendMessage(
                chat_ID, text=message, parse_mode="Markdown", reply_markup=keyboard
            )
